...
**Scope.** Every `CreateFunctionModule` + `UpdateFunctionModule` source body produced by sp4sap skills and agents.
...
When sp4sap writes FM source via `UpdateFunctionModule`, SAP parses the inline signature and updates TFDIR/FUPARAREF automatically. **There is no other way to set the signature** — no separate "parameters" endpoint, no `*"*"Local Interface:` auto-generation that writes into source.
...
When sp4sap creates an RFC-facing FM (PLM / WMS / external I/F), the completion report MUST flag the affected FMs with:
