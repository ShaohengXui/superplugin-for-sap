#!/usr/bin/env node
/**
 * sp4sap SessionStart hook — Legacy migration banner.
 *
...
function hasAnyProfile() {
  const dir = join(sp4sapHome(), 'profiles');
...
function main() {
  ...
  const message =
    '[sp4sap] 🆕 Multi-environment profiles are now available (Dev / QA / Prod).\n' +
    '         Your project still uses the legacy single-profile .sc4sap/sap.env.\n' +
    '         Run /sp4sap:sap-option to migrate — you choose the alias and tier.\n' +
