...
The 12 convention items in `phase6-review.md` are split into 4 buckets. Each bucket is reviewed by a separate `Agent({subagent_type: "sp4sap:sap-code-reviewer", model: "sonnet", ...})` dispatched in parallel.
...
```
Agent({
  subagent_type: "sp4sap:sap-code-reviewer",
  model: "sonnet",
...
| **1–2** | Dispatch `Agent({subagent_type: "sp4sap:sap-code-reviewer", model: "opus", ...})` **scoped only to the MAJOR findings** — pass finding list + affected object names. Opus re-reviews, attempts fix via `Update*` (up to 3 iterations per finding), writes final verdict to `review.md`. |
