...
- Store as `SAP_INDUSTRY=<key>` in `.sc4sap/sap.env` (step 4) **and** as `industry: "<key>"` in `.sc4sap/config.json` (step 10). Keep both in sync when changed via `/sp4sap:sap-option`.
- If the user is unsure, accept `other` and tell them they can change it anytime via `/sp4sap:sap-option`.
