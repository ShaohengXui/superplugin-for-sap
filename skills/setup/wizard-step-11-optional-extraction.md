...
- Optional. You can run `/sp4sap:setup spro` anytime later.
...
- If user answers no or skips: confirm "Skipped SPRO extraction. Run `/sp4sap:setup spro` later if needed." and continue to step 11b.
...
- Optional. You can run `/sp4sap:setup customizations` anytime later.
- 🔺 Initial scan runs several hundred MCP calls per module (one per
   standard exit + one per base table).
- ✅ Once cached, `/sp4sap:create-program` and `/sp4sap:analyze-symptom`
...
- If user answers no or skips: confirm "Skipped customization extraction. Run `/sp4sap:setup customizations` later if needed." and continue to step 12.
