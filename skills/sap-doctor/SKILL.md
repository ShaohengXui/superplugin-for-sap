...
<Purpose>
sp4sap:sap-doctor runs a structured health check across three layers: the SC4SAP plugin itself, the mcp-abap-adt MCP server registration, and the live SAP system connection. It surfaces problems with clear remediation steps so you can get back to development quickly.
</Purpose>
...
<Cache_Management>
`/sp4sap:sap-doctor` accepts two cache flags passed via `{{ARGUMENTS}}`:
...
MCP Server          [PASS]  plugin:sp4sap:sap responding, bridge preflight OK
...
Fix: Activate ZCL_S4SAP_CM_ALV, or re-run /sp4sap:setup wizard step 9b
     /sp4sap:sap-doctor --prune --yes   (reclaim 1.7 GB)
```
...
MCP Server          [FAIL]  plugin:sp4sap:sap not responding
...
Fix: Run /sp4sap:mcp-setup to install and register plugin:sp4sap:sap
```
</Output_Format>

<Remediation_Routing>
- Plugin version drift (cache ≠ marketplace) -> run `/reload-plugins`, then restart Claude Code
- MCP server missing -> `/sp4sap:mcp-setup`
- SAP connection error -> display connection troubleshooting from `/sp4sap:mcp-setup` Troubleshooting section
- Missing ZMCP_ADT_UTILS / ZMCP_ADT_DISPATCH / ZMCP_ADT_TEXTPOOL -> re-run `/sp4sap:setup` (wizard step 9a)
- Missing ZIF_S4SAP_CM / ZCX_S4SAP_EXCP / ZCL_S4SAP_CM_* -> re-run `/sp4sap:setup` (wizard step 9b)
- Any 9a/9b object inactive -> activate via ADT, or re-run the matching wizard step to reinstall source and activate
- Missing config -> run `/sp4sap:setup` wizard
- Authorization errors -> display required authorization objects (S_DEVELOP, S_TRANSPRT)
- Stale cache ≥ 500 MB -> `/sp4sap:sap-doctor --prune` to preview, then `/sp4sap:sap-doctor --prune --yes` to delete
